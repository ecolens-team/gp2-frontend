  ...
  return (
    <div className='bg-white border-b border-teal-100 p-3 sticky top-0 z-50 shadow-sm'>
      <div className='max-w-7xl mx-auto flex items-center justify-between gap-2 md:gap-4'>
        ...
        <div
          className={`relative flex-1 transition-all duration-300 md:ml-4 min-w-0 ${isSearchFocused ? 'max-w-none md:max-w-md' : 'max-w-[150px] sm:max-w-[200px] md:max-w-md'}`}
          ref={ref}
        >
              ...
              {results.length === 0 ? (
                ...
                results.map((obs) => (
                  ...
                  >
                    ...
                    {obs.image?.image && (
                      <img
                        src={obs.image.thumbnail ?? obs.image.image}
                        className='w-12 h-12 rounded-lg object-cover flex-shrink-0'
                        alt={obs.speciesName || 'Observation'}
                      />
                    )}
                  </div>
                ))
        ...
        <div
          className={`items-center gap-2 md:gap-4 shrink-0 ${isSearchFocused ? 'hidden md:flex' : 'flex'}`}
        >
          {authUser ? (
            <>
              <button
              ...
              <div className='flex items-center gap-2 md:gap-3 md:border-l border-gray-200 md:pl-4 md:ml-2'>
                ...
                >
                  {authUser.profile_picture ? (
                    <img
                      src={
                        authUser.profile_thumbnail ?? authUser.profile_picture
                      }
                      className='w-9 h-9 rounded-full'
                      alt={
                        authUser.first_name
                          ? `${authUser.first_name}'s profile`
                          : 'Profile'
                      }
                    />
                  ) : authUser?.first_name ? (
