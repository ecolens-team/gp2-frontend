...
export default function SpecializationPicker({
  ...
  useEffect(() => {
    ...
    api
      .get(`/species/taxonomy-options/?level=${selectedLevel.toLowerCase()}`)
      .then((res) =>
        setNameOptions(
          (res.data as string[])
            .filter(Boolean)
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      api
        .get(
          `/species/taxonomy-options/?level=${selectedLevel.toLowerCase()}&q=${nameInput}`,
        )
        .then((res) =>
          setNameOptions(
            (res.data as string[])
        ...
        <div className='flex gap-2'>
          ...
          <div className='flex-1'>
            <Select
              ...
              inputValue={
                STATIC_LEVELS.has(selectedLevel) || selectedLevel === 'CLASS'
                  ? undefined
                  : nameInput
              }
              onInputChange={
              ...
              placeholder={
                selectedLevel === 'CLASS'
                  ? 'Select class...'
                  : STATIC_LEVELS.has(selectedLevel)
                    ? `Select ${selectedLevel.toLowerCase()}...`
