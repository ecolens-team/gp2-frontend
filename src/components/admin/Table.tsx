...
const RowActions = ({
  ...
  const { mutate } = useMutation({
    mutationFn: (newStatus: string) =>
      api
        .patch(`admin/applications/${application.id}/review/`, {
  ...
  return (
    <div className='flex gap-2 justify-center'>
      <button
        className=' text-teal-600 bg-teal-500/30 rounded-xl p-2 text-sm font-bold flex gap-1 justify-center items-center hover:bg-teal-500/40 transition-colors'
        onClick={() => {
          mutate('APPROVED');
        }}
      >
      ...
      <button
        className=' text-red-500 bg-red-300/30 rounded-xl p-2 text-sm font-bold flex gap-1 justify-center items-center hover:bg-red-500/40 transition-colors'
        onClick={() => {
          mutate('REJECTED');
        }}
      >
...
export default function Table() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api
        .get(`/admin/applications/?status=${statusFilter}`)
        .then(
          (r) =>
            (Array.isArray(r.data)
              ? r.data
              : (r.data.results ?? [])) as ResearcherApplication[],
        ),
  });
