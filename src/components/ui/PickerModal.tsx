          ...
          {options.map((opt) => (
            ...
            >
              ...
              {value === opt.value && (
                <CheckCircle
                  size={18}
                  className='text-teal-500 shrink-0 ml-3'
                />
              )}
            </button>
          ))}
  ...
  return (
    ...
    >
      <div className='flex items-center gap-3'>
        ...
        <span
          className={`text-sm font-semibold ${value ? 'text-gray-700' : 'text-gray-400'}`}
        >
          {value || label}
        </span>
      </div>
      <span
        className={`text-xs font-black px-3 py-1 rounded-full transition-colors ${
          value ? 'bg-teal-100 text-teal-700' : 'bg-teal-600 text-white'
        }`}
      >
        {value ? 'Change' : 'Add'}
      </span>
