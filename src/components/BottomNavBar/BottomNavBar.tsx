import { Home, Map as MapIcon, Camera, Trophy, MessageCircle } from "lucide-react";
...
export default function BottomNavBar() {
    ...
    return(
        <nav className="fixed bottom-0 w-full bg-teal-50 border-t-2 border-teal-500/20 flex justify-around py-1.5 md:hidden text-lg shadow-[0_-4px_10px_rgba(0,0,0,0.0.2)] z-50">
            <NavLink to="/" className={({isActive}) => isActive ? "text-teal-600  flex flex-col items-center" : "text-gray-500 flex flex-col items-center"}>
                <Home />
                <span className="text-xs ">Explore</span>
            </NavLink>
            <NavLink to="/map" className={({isActive}) => isActive ? "text-teal-600  flex flex-col items-center" : "text-gray-500 flex flex-col items-center"}>
                <MapIcon />
            ...
            <NavLink to="/quests" className={({isActive}) => isActive ? "text-teal-600  flex flex-col items-center" : "text-gray-500 flex flex-col items-center "}>
                <Trophy />
                <span className="text-xs ">Quests</span>
            </NavLink>
            <NavLink to="/inbox" className={({isActive}) => isActive ? "text-teal-600  flex flex-col items-center" : "text-gray-500 flex flex-col items-center"}>
