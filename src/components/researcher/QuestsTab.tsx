...
const questSchema = z.object({
  ...
  description: z
    .string()
    .min(20, 'Please provide a description (20+ characters)'),
...
function SpeciesSearch({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      api
        .get(
          `/species/taxonomy-options/?level=species&q=${query}&with_ids=true`,
        )
        .then((r) =>
          setResults(
            (r.data as SpeciesOption[]).filter(
              (s) => !selected.find((x) => x.id === s.id),
            ),
          ),
        )
    ...
    <div className='space-y-2'>
      <label className='text-xs font-bold text-gray-500 block'>
        ...
        <span className='text-gray-400 font-normal'>
          (optional — leave empty for any species)
        </span>
      </label>
      ...
      {selected.length > 0 && (
        <div className='flex flex-wrap gap-2'>
          {selected.map((s) => (
            <span
              key={s.id}
              className='flex items-center gap-1.5 text-xs font-medium bg-teal-50 border border-teal-200 text-teal-800 px-2.5 py-1 rounded-full'
            >
...
function CreateQuestForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const inputClass =
    'w-full rounded-xl border border-gray-300 px-4 py-3 text-sm outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-200';

  return (
    <form
      onSubmit={handleSubmit((d) => mutation.mutate(d))}
      className='bg-teal-50/50 border border-teal-100 rounded-2xl p-6 space-y-5'
    >
      ...
      <div>
        <textarea
          ...
          placeholder='Describe the goal of this quest…'
          rows={3}
      ...
      <div className='grid grid-cols-2 gap-4'>
        <div>
          ...
          <input
            type='number'
            {...register('reward_pts', { valueAsNumber: true })}
            min={0}
            max={10000}
            className={inputClass}
          />
      ...
      <div>
        ...
        >
          ...
          {showMap
            ? 'Hide Zone Map'
            : 'Restrict to a Geographic Zone (optional)'}
        ...
        {showMap && (
          <div className='mt-3 space-y-1.5'>
            <p className='text-xs text-gray-400'>
              Draw a polygon on the map to restrict the quest to that area.
              Observations outside it won't count.
            </p>
            <DrawZoneMap onPolygonChange={setPolygon} />
            {polygon && (
              <p className='text-xs text-teal-600 font-medium flex items-center gap-1'>
                <CheckCircle size={12} /> Zone saved
              </p>
            )}
          </div>
        )}
  ...
  const daysLeft =
    quest.status === 'ACTIVE'
      ? Math.max(
          0,
          Math.ceil(
            (new Date(quest.end_date).getTime() - Date.now()) / 86400000,
          ),
        )
      : null;
  ...
  return (
    <div className='bg-white rounded-xl border border-gray-200 p-5'>
      <div className='flex items-start justify-between gap-3'>
        ...
        <span
          className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full shrink-0 ${cfg.color}`}
        >
          {cfg.icon} {cfg.label}
        </span>
      </div>
      ...
      <div className='flex items-center gap-4 mt-3 text-xs text-gray-400 font-medium flex-wrap'>
        <span className='flex items-center gap-1'>
          <Users size={12} /> {quest.participant_count ?? 0} participants
        </span>
        <span className='flex items-center gap-1'>
          <Target size={12} /> {quest.target_count} obs. needed
        </span>
        {daysLeft !== null && (
          <span
            className={`flex items-center gap-1 ${daysLeft <= 3 ? 'text-red-500 font-bold' : ''}`}
          >
            <Clock size={12} /> {daysLeft}d remaining
          </span>
        )}
        <span>
          {new Date(quest.start_date).toLocaleDateString()} –{' '}
          {new Date(quest.end_date).toLocaleDateString()}
        </span>
      </div>
    </div>
  );
        ...
        {isLoading ? (
          ...
          <div className='text-center py-16 text-gray-400'>
            ...
            <p className='text-sm mt-1'>
              Propose a quest to engage the community in your research area.
            </p>
          </div>
        ) : (
