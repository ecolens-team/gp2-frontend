...
import { AlertCircle, CheckCircle, Clock, FlaskConical, Inbox, ShieldAlert, Microscope, Trophy, MapPin } from "lucide-react";
...
const STATUS_CONFIG = {
  APPROVED: { label: "Approved", color: "bg-teal-100 text-teal-700", icon: <CheckCircle size={14} /> },
  PENDING:  { label: "Pending Review", color: "bg-amber-100 text-amber-700", icon: <Clock size={14} /> },
  REJECTED: { label: "Rejected", color: "bg-red-100 text-red-700", icon: <AlertCircle size={14} /> },
};
...
function ResearcherProfile({ authUser }: { authUser: IAuthUser }) {
  ...
  const appStatus = (profile?.application_status ?? "PENDING") as keyof typeof STATUS_CONFIG;
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex items-start gap-6">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className={`flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full ${statusCfg.color}`}>
            {statusCfg.icon} {statusCfg.label}
          </span>
        </div>
        <p className="text-sm text-gray-500 mt-0.5">@{authUser.username}</p>
        {profile?.institute && (
          <p className="text-sm text-gray-600 mt-1 font-medium">{profile.institute}</p>
        )}
        {specializations.length > 0 ? (
          <div className="flex flex-wrap gap-1.5 mt-3">
            {specializations.map(s => (
              ...
              >
                <span className="text-teal-400">{s.level[0] + s.level.slice(1).toLowerCase()}: </span>
                {s.name}
              </span>
            ))}
          </div>
        ) : (
          <p className="text-xs text-gray-400 mt-2 italic">No specializations set — add them to get a personalized verification queue.</p>
        )}
      </div>
    </div>
  );
...
export default function OverviewTab({
  ...
  const appStatus = (authUser.researcher_profile?.application_status ?? "PENDING") as keyof typeof STATUS_CONFIG;
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 mt-0.5">
                Your researcher application is {appStatus.toLowerCase()}. Full dashboard access will be enabled once an admin approves your application.
              </p>
            </div>
          </div>
        )}
      ...
      <div className="col-span-1">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 h-full min-h-[450px] flex flex-col">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-lg">Last 7 Days</span>
          </div>
...
function ActionCard({
  icon, label, count, description, onClick, disabled, accent, accentColor = "teal",
}: {
