...
const GOVERNORATES = [
  "Ajloun","Amman","Aqaba","Balqa","Irbid","Jerash",
  "Karak","Maan","Madaba","Mafraq","Tafilah","Zarqa",
];
...
export default function ExportTab() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await api.get(`/researcher/export/?${params.toString()}`, {
        responseType: "blob",
      });
  ...
  const inputClass = "w-full rounded-xl border border-gray-300 px-4 py-3 text-sm outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-200 bg-white";
  ...
  return (
    <div className="max-w-xl space-y-6">
      <div className="bg-white rounded-2xl border border-gray-100  p-6 space-y-5">
        ...
        {/* Taxonomy filter */}
        <div>
          <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-2">Filter by Taxonomy</label>
          <div className="bg-gray-50 rounded-xl p-4">
            <SpecializationPicker value={taxFilter} onChange={setTaxFilter} maxItems={1} />
            {taxFilter.length === 0 && (
              <p className="text-xs text-gray-400 mt-2">Leave empty to export all species in your specialization domain.</p>
            )}
          </div>
        </div>
        ...
        {/* Governorate */}
        <div>
          <label className="text-xs font-bold text-gray-500 block mb-1">Governorate</label>
          <select value={filters.governorate}
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          >
            <div className={`w-5 h-5 bg-white rounded-full shadow transition-transform ${filters.verified_only ? "translate-x-4" : "translate-x-0"}`} />
          </div>
          <span className="text-sm font-medium text-gray-700">Verified observations only</span>
        </label>
