...
function ConfidenceBadge({ value }: { value: number | null }) {
  if (value === null) return <span className="text-xs text-gray-400">No score</span>;
  const pct = Math.round(value * 100);
  const color = pct < 40 ? "bg-red-100 text-red-700" : pct < 70 ? "bg-amber-100 text-amber-700" : "bg-teal-100 text-teal-700";
...
function QueueList() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/researcher/queue/?page=${page}`).then(r => r.data),
  });
  ...
  if (!data?.results?.length) return (
    <div className="text-center py-16 text-gray-400">
      ...
      <p className="text-sm mt-1">Observations matching your specialization will appear here.</p>
    </div>
  );
      ...
      {data.results.map((item: QueueItem) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-gray-900 text-sm truncate italic">{item.species_name ?? "Unknown species"}</p>
            <p className="text-xs text-gray-500 mt-0.5">by {item.observer} · {item.governorate ?? formatLocation(item.latitude, item.longitude) ?? "Unknown location"}</p>
...
function ReportsList() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/researcher/reports/?page=${page}`).then(r => r.data),
  });

  const resolveMutation = useMutation({
    mutationFn: (reportId: number) => api.patch(`/researcher/reports/${reportId}/resolve/`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["researcher-reports"] }),
  });
  ...
  if (!data?.results?.length) return (
    <div className="text-center py-16 text-gray-400">
      ...
      <p className="text-sm mt-1">When users flag incorrect species labels, they'll appear here.</p>
    </div>
  );
    ...
    <div className="space-y-3">
      {data.results.map((report: ReportItem) => (
        <div key={report.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-sm text-gray-900 italic truncate">{report.predicted_species ?? "Unknown species"}</p>
            <p className="text-xs text-gray-500 mt-0.5">Reported by <span className="font-semibold">@{report.reporter}</span></p>
            {report.note && (
              <p className="text-xs text-gray-600 mt-1.5 bg-gray-50 rounded-lg px-3 py-2 italic">"{report.note}"</p>
            )}
          </div>
