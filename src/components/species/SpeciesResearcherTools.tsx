  ...
  const flags = [
    {
      key: 'is_endangered',
      label: 'Endangered',
      current: species.ecology.isEndangered,
    },
    {
      key: 'is_invasive',
      label: 'Invasive',
      current: species.ecology.isInvasive,
    },
    {
      key: 'is_endemic',
      label: 'Endemic to Jordan',
      current: species.ecology.isEndemic,
    },
  ];
  ...
  const nameFields = [
    {
      key: 'en',
      label: 'English',
      value: species.commonNameEn,
      draft: draftCommonNameEn,
      setDraft: setDraftCommonNameEn,
      payloadKey: 'common_name_en',
    },
    {
      key: 'ar',
      label: 'Arabic',
      value: species.commonNameAr,
      draft: draftCommonNameAr,
      setDraft: setDraftCommonNameAr,
      payloadKey: 'common_name_ar',
    },
  ];
      ...
      <div className='p-5 space-y-5'>
          ...
          <div className='flex flex-col gap-2'>
            {flags.map(({ key, label, current }) => (
              ...
              >
                ...
                >
                  <span
                    className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200 ${current ? 'translate-x-5' : 'translate-x-0'}`}
                  />
                </button>
              </div>
            ))}
        ...
        <div>
          ...
          <div className='space-y-2'>
            {nameFields.map(
              ({ key, label, value, draft, setDraft, payloadKey }) =>
                  ...
                  <div key={key} className='space-y-1.5'>
                    ...
                    <div className='flex gap-2'>
                      <button
                        disabled={updateMutation.isPending}
                        onClick={() =>
                          updateMutation.mutate({ [payloadKey]: draft })
                        }
                      ...
                      <button
                        ...
                        className='px-4 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 font-bold py-2 rounded-lg text-xs'
                      >
                        Cancel
                      </button>
                    </div>
        ...
        <div>
          ...
          {editingField === 'description' ? (
              ...
              <div className='flex gap-2'>
                ...
                >
                  {updateMutation.isPending ? (
                    ...
                    <>
                      <CheckCircle size={12} /> Save & Verify
                    </>
                  )}
                </button>
                <button
            ...
            <div className='bg-slate-50 rounded-lg px-4 py-3 border border-slate-100 flex justify-between items-start gap-3'>
              ...
              <button
                onClick={() => {
                  setDraftDescription(species.description.text || '');
