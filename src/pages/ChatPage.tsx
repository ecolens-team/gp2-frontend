...
const ChatPage: React.FC = () => {
  ...
  const [active, setActive] = useState<'conversations' | 'chat'>(
    'conversations',
  );
  ...
  useEffect(() => {
    api
      ...
      .then(() => {
        api.get<{ token: string }>('/ws-token/').then((res) => {
          setWsToken(res.data.token);
        });
      })
      .catch(() => {
        api
          .get<{ token: string }>('/ws-token/')
          .then((res) => {
            setWsToken(res.data.token);
          })
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(
      `${BASE_WS_URL}/ws/chat/${roomName}/?token=${wsToken}`,
    );
  ...
  const sendMessage = () => {
    if (
      ...
      setMessages((prev) => [
        ...prev,
        { sender: myId, content: messageText, timestamp: new Date() },
      ]);
  ...
  const groupedMessages = messages.reduce(
    (groups: { date: string; messages: ChatMessage[] }[], msg) => {
  ...
  return (
    <div className='flex h-full bg-[#f8fafc] font-sans'>
      {/* Sidebar */}
      <div
        className={`flex flex-col shrink-0 w-full md:w-70 bg-linear-to-tr from-[#14b8a6] to-[#bfdbfe] md:block ${active == 'chat' ? 'hidden' : 'block'}`}
      >
          ...
          {chats.map((chat: Chat) => (
            <div
              ...
              className={`flex items-center gap-3 p-3 mb-1 rounded-2xl cursor-pointer transition-all duration-200 ${
                selectedUser && selectedUser.id === chat.user.id
                  ? 'bg-white/25'
      ...
      {/* Main Chat */}
      <div
        className={`flex flex-col flex-1 overflow-hidden min-h-0 md:flex ${active == 'chat' ? 'flex' : 'hidden'}`}
      >
        {/* Header */}
        {selectedChat && (
          <div className='flex items-center gap-3 px-6 py-4 bg-white border-b border-slate-200'>
            ...
            {selectedUser && (
              <div
                className={`flex items-center justify-center w-11 h-11 text-base font-bold text-white rounded-full ${getAvatarColor(selectedUser.id)}`}
              >
            ...
            <div>
              ...
              <div className='flex items-center gap-1.5'>
                <div
                  className={`w-2 h-2 rounded-full ${connectionStatus === 'open' ? 'bg-green-500' : 'bg-slate-400'}`}
                />
          ...
          <div className='flex gap-3 px-6 py-5 bg-white border-t border-slate-200'>
            ...
            <button
              ...
              className={`flex items-center justify-center w-12 h-12 text-white rounded-full transition-all ${
                connectionStatus === 'open' && input.trim()
