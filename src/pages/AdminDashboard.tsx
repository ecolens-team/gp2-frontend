...
interface AdminStats {
  ...
  species: { total: number; with_observations: number; without_observations: number };
}

function StatPill({ label, value, accent }: { label: string; value: number; accent?: boolean }) {
  return (
    <div className={`flex justify-between items-center px-4 py-2 rounded-lg ${accent ? "bg-amber-50" : "bg-gray-50"}`}>
      <span className={`text-sm font-medium ${accent ? "text-amber-700" : "text-gray-500"}`}>{label}</span>
      <span className={`text-lg font-black ${accent ? "text-amber-600" : "text-gray-800"}`}>{value.toLocaleString()}</span>
    </div>
  );
}
          ...
          <div className="flex gap-4 flex-1">
            <div className="flex-1 flex flex-col justify-center gap-1 items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black text-2xl text-gray-900">Researchers</h3>
              </div>
              {stats.researchers.pending > 0 ? (
                ...
                <span className="text-xs text-gray-400 mt-1">No pending applications</span>
              )}
            </div>
            <div className="flex-1 space-y-2 flex flex-col justify-between">
              <div className="space-y-2">
                  <StatPill label="Approved" value={stats.researchers.approved} />
                  <StatPill label="Pending" value={stats.researchers.pending} accent={stats.researchers.pending > 0} />
              </div>
              <button
          ...
          <div className="flex gap-4 flex-1">
            <div className="flex-1 flex flex-col justify-center gap-1 items-center">
              ...
              {stats.quests.pending > 0 ? (
                ...
                <span className="text-xs text-gray-400 mt-1">No pending quests</span>
              )}
            </div>
            <div className="flex-1 space-y-2 flex flex-col justify-between">
              <div className="space-y-2">
                <StatPill label="Active" value={stats.quests.active} />
                <StatPill label="Pending" value={stats.quests.pending} accent={stats.quests.pending > 0} />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col justify-between">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-teal-600">{stats.species.with_observations.toLocaleString()}</span>
            </div>
            <div className="w-full h-1.5 bg-gray-100 rounded-full">
              <div
                className="h-full bg-teal-500 rounded-full"
                style={{ width: `${(stats.species.with_observations / stats.species.total) * 100}%` }}
              />
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Unobserved</span>
              <span className="font-bold text-gray-400">{stats.species.without_observations.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
...
const Sidebar = ({ setActive, active }: { setActive: (id: string) => void , active: string}) => {
    ...
    return (
        <div className="w-66 bg-teal-600 text-white h-full flex flex-col font-bold text-lg">
            <div className={`text-center bg-teal-700  p-3 font-black`}>ADMIN DASHBOARD</div>
            {tabs.map(tab => (
                <button key={tab.id} onClick={() => setActive(tab.id)} className={`border-b border-teal-500 p-3 px-6 ${active == tab.id? 'bg-teal-700/50' : 'bg-teal-600'} transition-colors`}>
                    {tab.title}
                </button>
            ))}
        </div>
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-gray-50">
            ...
            <div className="flex-1 overflow-y-auto p-8">
                <h1 className="text-2xl font-black text-gray-900 mb-6">{tabTitles[activeTab]}</h1>
