...
const inputClass =
  'w-full rounded-xl border-2 border-gray-200 px-4 py-3 text-base sm:text-sm outline-none transition focus:border-teal-500 focus:ring-4 focus:ring-teal-50';
...
function Register() {
  ...
  const userSchema = z
    .object({
      ...
      institute: z
        .string()
        .min(2, 'Please enter your university or organization')
        .optional(),
    ...
    <div className='min-h-dvh bg-white sm:bg-teal-50/60 flex flex-col items-center justify-center sm:py-10'>
      <div className='w-full max-w-md flex-1 sm:flex-none sm:bg-white sm:border sm:border-teal-100 p-6 sm:p-8 sm:rounded-2xl sm:shadow-sm space-y-6'>
        <header className='mt-4 sm:mt-0'>
          <h2 className='font-black text-teal-800 text-3xl tracking-tight'>
            EcoLens
          </h2>
        ...
        <div className='flex gap-3'>
          ...
          >
            <SproutIcon
              size={26}
              className={!isResearcher ? 'text-teal-600' : 'text-gray-400'}
            />
            <span className='font-bold text-sm text-center leading-tight'>
              Nature Enthusiast
            </span>
            <span className='font-normal text-[11px] text-center leading-tight opacity-70'>
              Capture findings, explore biodiversity and connect with other
          ...
          >
            ...
            <span className='font-normal text-[11px] text-center leading-tight opacity-70'>
              Verify species data, view distribution stats and organize data
              collection events
            </span>
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit(handleRegister)} className='space-y-3.5'>
          ...
          <div>
            ...
            />
            {errors.phoneNumber && (
              <span className={errorClass}>{errors.phoneNumber.message}</span>
            )}
          </div>
          ...
          {isResearcher && (
            <div className='bg-teal-50/50 p-4 rounded-xl border border-teal-100 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300'>
              ...
              <div>
                ...
                />
                {errors.institute && (
                  <span className={errorClass}>{errors.institute.message}</span>
                )}
              </div>
              ...
              <div>
                <div className='flex items-center gap-2 mb-2'>
                  ...
                  <div className='group relative'>
                    <span className='cursor-help rounded-full bg-teal-100 px-1.5 py-0.5 text-[10px] font-bold text-teal-700'>
                      ?
                    </span>
                    <div className='invisible group-hover:visible absolute bottom-full left-0 mb-2 w-64 rounded-lg bg-gray-800 p-2 text-[11px] leading-relaxed text-white shadow-xl z-50'>
                      To verify your Researcher status, please upload a document
                      such as a University ID, or a CV highlighting your
                      flora/fauna expertise.
          ...
          <div className='pt-2 space-y-4'>
            ...
            <p className='text-[15px] text-center text-gray-500 pb-6 sm:pb-0'>
              ...
              <Link to='/login' className='font-bold text-teal-600 ml-1'>
                Log in
              </Link>
            </p>
          </div>
