...
export default function Login() {
  ...
  const onSubmit = async (data: LoginForm) => {
    try {
      await login({ username: data.username.trim(), password: data.password } as any);
    ...
    <div className="min-h-dvh bg-white sm:bg-teal-50/60 flex flex-col sm:items-center sm:justify-center">
      ...
      <div className="w-full max-w-sm flex-1 sm:flex-none flex flex-col justify-between sm:justify-start sm:rounded-2xl sm:border sm:border-teal-100 sm:bg-white p-6 sm:shadow-sm sm:my-8">
          ...
          <header className="mb-8 mt-4 sm:mt-0">
            <h1 className="text-3xl font-black text-teal-700 tracking-tight">EcoLens | Login</h1>
            <p className="text-gray-500 mt-2 ">Welcome back!</p>
          </header>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              <label htmlFor="username" className="mb-1.5 block text-[13px] font-semibold text-gray-700 ml-1">
                Email or Username
              </label>
              <input
                {...register("username")}
              ...
              />
              {errors.username && <p className="mt-1.5 text-xs text-red-500 font-medium ml-1">{errors.username.message}</p>}
            </div>

            <div>
              <label htmlFor="password" className="mb-1.5 block text-[13px] font-semibold text-gray-700 ml-1">
                Password
              </label>
              <input
                {...register("password")}
        ...
        <div className="mt-10 sm:mt-8 space-y-4">
          ...
          <div className="text-[15px] text-center text-gray-500 pb-4 sm:pb-0">
            Don't have an account? <Link to={'/register'} className="font-bold text-teal-600 ml-1">Register</Link>
          </div>
        </div>
