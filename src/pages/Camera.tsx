...
import {
  ...
  PickerTrigger,
  type PickerOption,
} from '../components/ui/PickerModal';
...
export default function AddObservation() {
  ...
  const [step, setStep] = useState<'CAMERA' | 'DETAILS'>(
    window.innerWidth > 768 ? 'DETAILS' : 'CAMERA',
  );
  ...
  const [openPicker, setOpenPicker] = useState<
    'weather' | 'governorate' | 'quest' | null
  >(null);
        ...
        try {
          ...
          toast.error(
            err?.response?.data?.error ??
              'Observation created but quest assignment failed',
          );
        }
      }
      if (obs) navigate('/');
  ...
  async function runAI(image: File) {
    ...
    try {
      ...
      setIsPredicting(false);
      toast.error(
        err?.response?.data?.error || err?.message || 'AI prediction failed',
      );
    }
  }
    ...
    return (
      <div className='fixed inset-0 z-150 bg-black flex flex-col items-center'>
        <div className='flex justify-between items-center p-4 text-white z-10 bg-linear-to-b from-teal-900/40 to-transparent absolute top-0 w-full'>
          <button
            onClick={() => navigate(-1)}
            className='p-2 bg-teal-900/40 rounded-full backdrop-blur-lg'
          >
            <ArrowLeft size={24} />
          </button>
  ...
  return (
    <div className=' overflow-y-auto flex flex-col pb-24 max-w-4xl mx-auto bg-white'>
      ...
      <div className='border-b-2  border-teal-600/20 mb-4'>
        ...
        <div className='flex gap-4 m-4'>
          {previewUrls.map((url, index) => (
            <button
              key={index}
              className={`${index == selectedImage ? 'border-3 border-teal-400/40' : 'border-2 border-teal-600/20'} w-24 h-24 rounded-3xl bg-gray-200 text-gray-400 flex justify-center items-center active:scale-95 transition-transform`}
              onClick={() => {
          ...
          <button
            className='w-24 h-24 border-2 border-teal-600/20  rounded-3xl bg-gray-200 text-gray-400 flex justify-center items-center active:scale-95 transition-transform'
            onClick={() => {
              setStep(window.innerWidth > 768 ? 'DETAILS' : 'CAMERA');
            }}
          >
            <Plus size={30} />
          </button>
          ...
          {isPredicting ? (
            ...
            <div className='flex justify-between items-center'>
              ...
              <div className='bg-teal-50 border border-teal-200 text-teal-700 font-bold px-3 py-1.5 rounded-full'>
                {aiPrediction?.confidence &&
                  (aiPrediction?.confidence * 100).toFixed(2)}
                %
              </div>
            </div>
          )}
      ...
      <div className='px-4 flex flex-col gap-3 mb-8'>
        <p className='text-xs font-semibold text-gray-400 uppercase tracking-wider pl-2 mt-4'>
          Optional Data
        </p>
        <PickerTrigger
        ...
        {joinedQuests.length > 0 && (
          <PickerTrigger
            ...
            value={
              joinedQuests.find((q) => String(q.id) === selectedQuestId)
                ?.title ?? ''
            }
