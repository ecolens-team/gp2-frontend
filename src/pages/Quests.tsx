...
import { getQuests, getGamificationProfile, type GamificationBadge } from '../services/questService';
import { Leaf, Target, Camera, ChevronRight, Trophy, Users, Lock, Star, CameraIcon } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import Spinner from '../components/ui/Spinner';
import type { Quest } from '../interfaces/quest';

const BadgeIcon = ({ badge, size = 40 }: { badge: GamificationBadge; size?: number }) => {
  if (badge.icon) {
    return <img src={badge.icon} alt={badge.name} style={{ width: size, height: size }} className="object-contain" />;
  }
  switch (badge.criteria_type) {
    case 'OBSERVATION_COUNT': return <CameraIcon size={size * 0.5} />;
    case 'QUEST_COUNT': return <Trophy size={size * 0.5} />;
    case 'FIRST_SPECIES': return <Star size={size * 0.5} />;
    default: return <Trophy size={size * 0.5} />;
  }
};
    ...
    >
      ...
      <div className="space-y-3">
        ...
        <div className="flex justify-between items-center">
          <span className="text-xs font-bold text-gray-400">{quest.progressPercent}% Completed</span>
      ...
      <div className="max-w-3xl mx-auto p-4 md:p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="bg-white p-3 rounded-2xl border border-teal-100 flex items-center gap-2">
            ...
            <span className="font-black text-teal-600">{activeQuests.length} joined</span>
          </div>
        </div>
        ...
        <div className="bg-linear-to-r from-teal-600/80 to-sky-700/50 rounded-3xl p-6 text-white mb-6 shadow-sm relative overflow-hidden">
          <div className="relative z-10">
            ...
            <h2 className="text-4xl font-black mb-1">
              ...
              {profile && <span className="text-xl font-bold text-teal-200 ml-2">{profile.level_name}</span>}
            </h2>
            <p className="text-teal-200 text-xs mb-3">{profile?.total_pts.toLocaleString() ?? 0} XP total</p>
            <div className="flex justify-between text-xs font-bold mb-1.5">
              <span>Progress to Level {profile ? profile.level + 1 : '—'}</span>
              <span>{profile?.xp_in_level ?? 0} / {profile?.xp_per_level ?? 1000} XP</span>
            </div>
            <div className="w-full bg-white/20 h-2 rounded-full overflow-hidden">
              <div
                className="bg-white h-full rounded-full transition-all duration-700"
                style={{ width: `${profile ? (profile.xp_in_level / profile.xp_per_level) * 100 : 0}%` }}
              />
            </div>
          </div>
            ...
            <div className="bg-white rounded-3xl p-5 border-2 border-teal-600/10">
              <div className="flex flex-wrap gap-4">
                {profile.badges.map(badge => (
                  <div key={badge.id} className="flex flex-col items-center gap-1.5 w-16" title={badge.description || badge.name}>
                    <div className={`w-15 h-15 rounded-full flex items-center justify-center transition-all ${
                      badge.earned
