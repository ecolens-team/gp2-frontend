...
import { getSpeciesById, getObservations } from "../services/observationsService";
...
export default function SpeciesProfile() {
  ...
  const isResearcher = authUser?.role === 'RESEARCHER' || authUser?.role === 'ADMIN';
  ...
  const obsWithCoords = observations.filter(o => o.latitude != null && o.longitude != null);
          ...
          <div className="lg:col-span-7 space-y-10">
            ...
            <section className="space-y-6">
              ...
              {!species.description.isVerified && (
                <div className="bg-amber-50 border-l-4 border-amber-400 p-5 md:p-6 rounded-xl flex items-center gap-3">
                  ...
                  <p className="text-sm text-amber-800">Description not yet verified by a researcher.</p>
                </div>
              )}
            ...
            <section className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Top Governorates</p>
                  {species.ecology.primaryGovernorates?.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-400 italic">No observation data yet.</p>
                  )}
                </div>
                ...
                <div className="pt-6 border-t border-gray-50">
                  ...
                  <div className="space-y-4">
                    {species.community.topObservers.map((obs, idx) => (
                      <div key={obs.name} className="flex items-center gap-4 group cursor-pointer" onClick={() => navigate(`/users/${obs.name}`)}>
                  ...
                  {species.community.activeQuests.map((quest) => (
                    <div key={quest.id} className="p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-colors cursor-pointer border border-white/10 group">
                      ...
                      <div className="w-full h-1.5 bg-black/20 rounded-full overflow-hidden">
                        <div className="h-full bg-teal-400 rounded-full" style={{ width: `${quest.progressPercent}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
